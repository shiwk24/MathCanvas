...
class PackedDataset(torch.utils.data.IterableDataset):
                    ...
                    if self.is_mandatory[group_index]:
                        while True:
                            ...
                            if num_tokens < self.max_num_tokens_per_sample:
                                sequence_status = self.pack_sequence(sample, sequence_status, force_keep_vit=True)
    ...
    def pack_sequence(self, sample, sequence_status, force_keep_vit=False):
        ...
        survivor_index = -1
        if force_keep_vit:
            vit_image_indices_in_plan = [
                i for i, item in enumerate(sequence_plan) if item['type'] == 'vit_image'
            ]
            if vit_image_indices_in_plan:
                survivor_index = random.choice(vit_image_indices_in_plan)
        ...
        for plan_index, item in enumerate(sequence_plan):
            ...
            if item['type'] == 'text':
                ...
                is_survivor = (plan_index == survivor_index)
                if not is_survivor and item['enable_cfg'] == 1 and random.random() < self.data_config.vit_cond_dropout_prob:
                    curr_rope_id += 1
                    continue
